...
{
	printf ("\nError: Qmark requires a floating-point processor\n");
	exit (0);
}
...
int Qm_Test_QmPointTransformNi( void  ) {
...
int Qm_Test_QmPointTransformFx( void  ) {
	float oldtime, newtime, targettime, totaltime;
	int score, i, iters;
	vec3_t in = { 1.3, 2.4, 3.5 }, out = { 4.6, 5.7, 6.8 };
...
int Qm_Test_TransformVector( void  ) {
	float oldtime, newtime, targettime, totaltime;
	int score, i, iters;
	vec3_t in = { 1.3, 2.4, 3.5 }, out = { 4.6, 5.7, 6.8 };
...
int Qm_Test_DrawParticle( void ) {
	...
	particle.org[0] = -221.67;
	particle.org[1] = -1504.08;
	particle.org[2] = 95.83;
	particle.color = 0x5a;
...
{
	double			time, oldtime, newtime, targettime, totaltime;
	extern void (*dos_error_func)(char *, ...);
	static	char	cwd[1024];
	int i, iters;
	vec3_t in = { 1.3, 2.4, 3.5 }, out = { 4.6, 5.7, 6.8 };

	int PointTransformNiScore, PointTransformFxScore;

	printf ("Qmark%i -%s%s v%4.2f built with %s\n", SUBARCH, OLEVEL, LTOFLAGS, VERSION, CCVERSION);
